...
Compositer::Compositer(QOpenGLFunctions_4_3_Core* func, int swidth, int sheight) :m_func(func)
{
	...
	m_func->glGenRenderbuffers(1, &m_colorMSAA);
	m_func->glBindRenderbuffer(GL_RENDERBUFFER, m_colorMSAA);
	m_func->glRenderbufferStorageMultisample(GL_RENDERBUFFER, 4, GL_RGBA8, swidth, sheight);
	m_func->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_colorMSAA);
